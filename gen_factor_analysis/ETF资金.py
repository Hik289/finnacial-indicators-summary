...
class ETF_cap():
    ...
    def __init__(self):
        ...
        # 基金份额
        self.float_share = pd.read_pickle(file_path +"/data/daily/fund/FUNDSHARE")
        ...
        # 数据日期序列重整
        self.float_share = self.float_share.fillna(0).reindex(index=self.daily_dates).ffill().dropna()
        self.fund_close = self.fund_close.fillna(0).reindex(index=self.daily_dates).ffill().dropna()
    ...
    def get_index_comp(self, idx_name):
        ...
        if len(idx_info) == 0:            
            ...
            start_date = np.nanmin(all_date_array)
    ...
    # 个股ETF资金流计算
    # -------------------------------------------------------------------------
    def get_stock_ETF_money(self, ETF_code):
...
if __name__ == "__main__":
    ...
    #     print(ETF_code)        
    #     stock_money = model.get_stock_ETF_money(ETF_code)        
    #     money_sum = money_sum.add(stock_money, fill_value=0)
    
    
    money_sum = pd.read_pickle('stock_ETF_money') * 10000
    ...
    for data_type in ['orig', 'float', 'amount']:     # 是否除以流通市值 
        for freq in ['W','M']:   # 信号发出频率
            for method in ['orig', 'qoq', 'yoy']:  # 信号调整
                ...
                data_monneyflow.to_pickle('ETF_results/ETF_money_{}_{}_{}'.format(data_type, freq, method))
