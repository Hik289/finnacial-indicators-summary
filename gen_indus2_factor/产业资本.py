...
class indus_cap():
    ...
    # 获取行业标签
    # ================================================================================
    def get_indus_data(self, origin_data):
                    
        # 数据复制
        data = origin_data.copy()
                
        # 提取有行业归属和收盘价数据的日期
        data = data.loc[data.loc[:, '统计日期'] > pd.to_datetime('2005-01-01'),:]
        
        # 记录日期  
        for index in data.index:
            
            if data.loc[index, '统计日期'] <= self.indus_belong.index[-1]:
                
                # 记录行业归属
                data.loc[index, '所属行业'] = \
                self.indus_belong.loc[data.loc[index, '统计日期'], data.loc[index, '股票代码']]
                
            else:
                # 如果日期超出了行业归属矩阵的最新日期，就提取最新的行业归属数据
                data.loc[index, '所属行业'] = \
                self.indus_belong.loc[self.indus_belong.index[-1], data.loc[index, '股票代码']]
                
                
        # 剔除所属行业为空数据
        data = data.loc[~data.loc[:, '所属行业'].isnull(),:]
        data = data.loc[~(data.loc[:, '所属行业']=='nan'),:]
    ...
    def gen_factor(self, data_name, data, period_type, data_type, freq):
        ...
        if (data_name == 'AShareFreeFloatCalendar') & (period_type == 'next'):
            ...
            # 分组聚合
            data_group = data.groupby(['统计日期','所属行业']).sum()['资金数据'].reset_index()
        
            # 转换成行业数据
            indus_money = data_group.pivot(index='统计日期', columns='所属行业')['资金数据']
                
            # 列名和日期重置
            indus_money = indus_money.reindex(columns=self.indus_info['行业名称']).resample('D').last()
    ...
    def gen_future_calendar(self, data, freq):
        ...
        # 日期频率
        resample_date = self.daily_dates.resample(freq).last().loc['2015-01-01':].dropna()
        
        # 记录行业数据
        indus_data = pd.DataFrame(np.nan, index=resample_date.values, columns=self.indus_info['行业名称'])
        ...
        for index in resample_date.index:
            ...
            if len(reselect_data) == 0:
                ...
                stock_indusbelong = reselect_data.groupby(['股票代码', '限售股上市日期']).last()['所属行业']
                
                # 终版数据
                tmp_data = pd.concat([stock_volumes, stock_indusbelong], axis=1).reset_index()
                tmp_data = tmp_data.set_index('股票代码')
                ...
                # 结果存储
                indus_data.loc[resample_date[index], :] = tmp_data.groupby(['所属行业']).sum()['资金数据']
    ...
    def gen_future_planhold(self, data, freq):
        ...
        for index in resample_date.index:
            ...
            if len(keep_data) > 0:
                ...
                # 相同股票合并
                stock_volumes = reselect_data.groupby(['股票代码']).sum()['拟变动数量上限']
                stock_indusbelong = reselect_data.groupby(['股票代码']).last()['所属行业']
    ...
    for index in settings_list.index:
        ...
        for data_type in ['orig', 'float', 'amount']:
            for freq in ['W', 'M']:
                ...
                # 数据计算和存储
                factor = model.gen_factor(data_name, data, period_type, data_type, freq)
                
                # 产业资本数据经常为空值，0值直接置为空
                factor[factor==0] = np.nan
                
                # 结果存储
                factor.to_pickle('industry_results/{}_{}_{}_{}_{}'.format(
