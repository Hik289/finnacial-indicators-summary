    ...
    for data_type in ['orig', 'float', 'amount']:     # 是否除以流通市值 
        for freq in ['W','M']:   # 信号发出频率
            for method in ['orig', 'qoq', 'yoy']:  # 信号调整
                ...
                # 北向流入资金统计
                data_moneyflow = model.gen_money_change(data_type, freq, method)
                data_moneyflow.to_pickle('north_results/north_change_{}_{}_{}'.format(data_type, freq, method))
