...
if __name__ == "__main__":
    ...
    # # 两融数据读取
    # market_borrow = model.borrow.sum(axis=1).resample('M').sum()/1E8 # 融资买入额
    # market_repay = model.repay.sum(axis=1).resample('M').sum()/1E8     # 融资偿还额
    # market_net_buy = model.net_buy.sum(axis=1).resample('M').sum()/1E8 
    
    # market_tr_balance = model.tr_balance.sum(axis=1).resample('M').last()/1E8 
    # market_slend_balance = model.slend_balance.sum(axis=1).resample('M').last()/1E8 

    # data = pd.concat([market_tr_balance, market_slend_balance, market_borrow, market_repay, market_net_buy], axis=1)
