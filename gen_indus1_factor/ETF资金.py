# -*- coding: utf-8 -*-
"""
Created on Sun Sep 26 16:56:45 2021
...
class ETF_cap():
    ...
    def __init__(self):
        ...
        # 读取股票列表
        self.stock_info = pd.read_pickle(file_path + '/data/basic/stock_info')    
        ...
        # 获取一级行业收盘价
        self.indus_close = pd.read_pickle(file_path + '/data/daily/indus/indus1_close')
        ...
        # 指数持仓
        self.index_member = pd.read_pickle(file_path + '/data/daily/stock/AShare_Index_member')
        ...
        # 基金单位净值和福泉精致
        self.fund_close = pd.read_pickle(file_path +"/data/daily/fund/F_NAV_UNIT")
        self.fund_adjclose = pd.read_pickle(file_path +"/data/daily/fund/F_NAV_ADJUSTED")
                
        # 基金份额
        self.float_share = pd.read_pickle(file_path +"/data/daily/fund/ETF_share")
                
        # ETF列表
        self.ETF_info = pd.read_pickle(file_path + "/data/basic/ETF_fund_info")
        self.indus_ETF_list = pd.read_excel(file_path + '/data/basic/ETF基金列表.xlsx', index_col=0)
...
#     # 中间过程数据存储
#     money_sum.to_pickle('tmp_data/allETF_money')
#     money_sum = pd.read_pickle('tmp_data/allETF_money') * 10000
...
#                 data_monneyflow.to_pickle('ETF_results/allETF_{}_{}_{}'.format(data_type, freq, method))
...
#     # # 中间过程数据存储
#     money_sum.to_pickle('tmp_data/indus_ETF_money')        
#     money_sum = pd.read_pickle('tmp_data/indus_ETF_money') * 10000
...
#                 # 北向流入资金统计
#                 data_monneyflow = model.gen_money_change(money_sum, data_type, freq, method)
#                 data_monneyflow.to_pickle('ETF_results/indusETF_{}_{}_{}'.format(data_type, freq, method))
...
# =============================================================================
    ...
    # 中间过程数据存储
    money_sum.to_pickle('tmp_data/allETFselect_money')
    money_sum = pd.read_pickle('tmp_data/allETFselect_money') * 10000
    ...
    for data_type in ['orig', 'float', 'amount']:     # 是否除以流通市值 
        for freq in ['W','M']:   # 信号发出频率
            for method in ['orig', 'qoq', 'yoy']:  # 信号调整
                ...
                # 北向流入资金统计
                data_monneyflow = model.gen_money_change(money_sum, data_type, freq, method)
                data_monneyflow.to_pickle('ETF_results/allETFselect_{}_{}_{}'.format(data_type, freq, method))
...
# =============================================================================
    ...
    # # 中间过程数据存储
    money_sum.to_pickle('tmp_data/indusETFselect_money')        
    money_sum = pd.read_pickle('tmp_data/indusETFselect_money') * 10000
    ...
    for data_type in ['orig', 'float', 'amount']:     # 是否除以流通市值 
        for freq in ['W','M']:   # 信号发出频率
            for method in ['orig', 'qoq', 'yoy']:  # 信号调整
                ...
                # 北向流入资金统计
                data_monneyflow = model.gen_money_change(money_sum, data_type, freq, method)
                data_monneyflow.to_pickle('ETF_results/indusETFselect_{}_{}_{}'.format(data_type, freq, method))
...
# #     # money_sum = pd.read_pickle('tmp_data/allETF_money') * 10000    
# #     # # money_sum = pd.read_pickle('tmp_data/indus_ETF_money') * 10000
# #     # # b = money_sum.sum(axis=1)
# #     # data_monneyflow = model.gen_money_change(money_sum, 'orig', 'M', 'orig')
